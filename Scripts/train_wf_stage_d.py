"""
Stage D Training: Two-phase linear probe + gating network learning.

Phase 1: Train linear probe (classifier head) with frozen encoder
Phase 2: Freeze linear probe, train gating network for w(a,x,t)

Model: final_logits = audio_logits / T + w(a,x,t) * log(prior + eps)

Key differences from Stage C:
- Uses gating network instead of scalar w
...
def train_phase2(
    ...
    """
    Phase 2: Freeze linear probe, train gating network and fusion parameters.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage D: Linear probe + gating network training")
